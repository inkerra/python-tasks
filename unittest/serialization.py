...
class ListOfListSomeElements(unittest.TestCase):
    ...
    def some_others(self):
	eq_([']am]', [1]], deserialize("[<4=]am]>[<int=1>]]"))
        eq_([[1], ["ss", 2], [[[[1]]], [2,[3,"12"]]]], deserialize("[[<int=1>][<2=ss><int=2>][[[[<int=1>]]][<int=2>[<int=3><2=12>]]]]"))
	eq_([']am]', [[[1]]]], deserialize("[<4=]am]>[[[<int=1>]]]]"))
	eq_((['<am>', [1]], deserialize("[<4=<am>>[<int=1>]]")))
	eq_(['same', []], deserialize("[<4=same>[]]"))
	eq_([110, '1'], deserialize("[<int=110><1=1>]"))
	eq_(['add', 2, 3], deserialize("[<3=add><int=2><int=3>]"))
	eq_([[[],1,[0],"abacaba"], 110], deserialize("[[[]<int=1>[<int=0>]<7=abacaba>]<int=110>]"))
	eq_("[<int=0>[<int=1><int=2>]<3=abc>]", serialize([0, [1, 2], "abc"]))

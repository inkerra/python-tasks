...
def get_content(url):
    try:
        ...
        print "Warning: can't open {}".format(url)
        return ""
    
def loader(param_q, res_q, func):
    while 1:
        param = param_q.get()
...
def load_pages(max_thread_pool_sz, *urls):
    ...
    loader_params = (param_q, res_q, get_content)
    loaders = []
    ...
    for i in range(max_thread_pool_sz):
        th = threading.Thread(None, loader, "loader-{}".format(i), loader_params)
        th.daemon = True
        th.start()
        loaders.append(th)
...
if __name__ == "__main__":
    ...
    for url, page in load_pages(5, *urls):
        try:
            print url, len(page)
        except:
            pass
